<template class="app-container">
  <div>
    ...
    <el-dialog :visible.sync="dialogTableVisible">
      <SupList :Multiple="Multiple" :type="type" ref="dList" />
    </el-dialog>
  </div>
</template>
      ...
      if (data && data.length > 0) {
        if (Array.isArray(data)) {
          ...
          for (var i = 0; i < data.length; i++) {
            str += data[i].supplierName + (i + 1 == data.length ? '' : '/');
            this.formdata.push(data[i].supplierId);
          }
          this.chooseVal = str;
