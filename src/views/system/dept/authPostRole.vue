...
<script>
import { getAuthRole, updateAuthRole } from '@/api/system/post';
...
export default {
  ...
  created() {
    ...
    if (postId) {
      this.loading = true;
      getAuthRole(postId).then((response) => {
        this.form = response.post;
  ...
  methods: {
    ...
    submitForm() {
      ...
      updateAuthRole({ postId: postId, roleIds: roleIds }).then((response) => {
