<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '开票记录管理', '开票记录列表', '编辑开票记录']" />
...
import { invoiceInfo, invoiceEdit } from '@/api/data/produce';
import { invoiceAmount } from '@/api/data/produce';
import { custInfo } from '@/api/data/customer';
export default {
  name: 'BillingRequestEdit', //编辑开票记录
  dicts: ['invoice_type'],
  ...
  data() {
    return {
      ...
      ruleData: {
        ...
        invoicingDate: [{ required: true, message: '请选择开票日期', trigger: 'blur' }],
        userName: [{ required: true, message: '请选择开票记录人', trigger: 'change' }],
  ...
  methods: {
    ...
    getTotalAmount(id, prjNumber) {
      invoiceAmount({ contId: id }).then((response) => {
