    ...
    <div class="appi">
      <div class="appc">
        ...
        <div class="main">
          ...
          <div class="p2">
            <el-button v-hasPermi="['produce:payment1:add']" type="primary" @click="addC"
              >添加</el-button
            >
            ...
            <el-button type="warning" v-hasPermi="['produce:payment1:import']" @click="importList"
              >导入</el-button
            >

            <el-button
              v-hasPermi="['produce:payment1:importTemplate']"
              style="width: auto"
              type="primary"
              @click="downTemplate"
              >下载导入模版</el-button
            ...
            <!-- <el-button
              v-hasPermi="['produce:payment1:remove']"
              type="danger"
              @click="deleteT"
          ...
          <div class="p3">
            ...
            >
              ...
              <el-table-column prop="id" label="收款编号"></el-table-column>
              <el-table-column prop="contName" label="合同名称"></el-table-column>
              <el-table-column prop="prjName" label="收款方式">
              ...
              <el-table-column prop="nodeName" label="收款节点"> </el-table-column>
              <el-table-column prop="nodeName" label="此次收款金额(元)">
                <template slot-scope="scope">
              ...
              <el-table-column fixed="right" label="操作" width="200">
                <template slot-scope="scope">
                  <el-button
                    v-hasPermi="['produce:payment1:query']"
                  ...
                  >
                  <el-button
                    v-hasPermi="['produce:payment1:edit']"
                    type="text"
                    size="small"
...
<script>
import { receiptsList, receiptsDelete, importTemplate2 } from '@/api/data/produce';
export default {
  name: 'IncomeRecordCheck', //收款记录列表
  dicts: ['payment_type'],
  data() {
    return {
      loading: true, // 遮罩层
  ...
  methods: {
    ...
    getList() {
      receiptsList(this.queryParams).then((response) => {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/production-d/incomeRecordInfo', query: { id: row.id } });
    },

    //编辑
    handleEdit(row) {
      console.log(row);
      this.$router.push({ path: '/production-d/incomeRecordEdit', query: { id: row.id } });
    },

    //添加
    addC() {
      this.$router.push({ path: '/production-d/incomeRecordAdd' });
    },
    ...
    async deleteT(row) {
      ...
      receiptsDelete(row.id).then((res) => {
        if (res.code == 200) {
      ...
      receiptsDelete(params.join(',')).then((res) => {
        if (res.code == 200) {
          this.$message.success(`删除成功`);
          this.$refs.multipleTable.clearSelection();
          this.getList();
