<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '收款记录管理', '收款记录列表', '编辑收款记录']" />
...
import { receiptsEdit, receiptsInfo, payAmount } from '@/api/data/produce';
import { proPayMentNodeList, payNodeAmount } from '@/api/data/project';
import { receiptsAmount } from '@/api/data/produce';
import { custInfo } from '@/api/data/customer';
export default {
  name: 'IncomeRecordEdit', //编辑收款记录
  ...
  methods: {
    getData() {
      if (this.$route.query.id) {
        ...
        receiptsInfo(this.$route.query.id).then((response) => {
    ...
    //   receiptsAmount({ contId: id, prjNumber: prjNumber }).then((response) => {
