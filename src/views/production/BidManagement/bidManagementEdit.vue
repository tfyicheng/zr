<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '投标管理', '投标列表', '编辑投标']" />
            ...
            <div class="p0">
              <el-form :model="form" :inline="true" ref="form5" :rules="ruleData">
                <el-form-item label="代 理 名 称">
                  <ClieChoose
                    :data.sync="form.agentId"
                    :Multiple="false"
                    :backVal="form.agentName"
                    @getClie="getClie2"
                  />
                </el-form-item>
            ...
            <div class="p0">
              <el-form :model="form" :inline="true" ref="form4" :rules="ruleData">
                <el-form-item label="项 目 负 责 人">
                  <PersonChoose :data.sync="form.prjLeader1" :backVal="form.prjLeaderName" />
                </el-form-item>

                <el-form-item label="技 术 负 责 人">
                  <PersonChoose :data.sync="form.techLeader1" :backVal="form.techLeaderName" />
                </el-form-item>
              </el-form>
            </div>
          </div>
              ...
              <div class="m2mt">
                ...
                <div class="m2mb" style="max-width: 250px">
                  <el-form-item label="" prop="userIds">
                    <PersonChoose :data.sync="c.userIds" :backVal="c.userNames" />
                  </el-form-item>
                </div>
...
<script>
import { bidInfo, bidEdit } from '@/api/data/produce';
    ...
    getInfo() {
      if (this.$route.query.id) {
        bidInfo(this.$route.query.id).then((response) => {
          ...
          // if (response.data.guaranteeAmount == null) {
          //   this.form.guaranteeAmount = undefined;
          // }
          // if (response.data.totalInvestment == null) {
          //   this.form.totalInvestment = undefined;
          // }
          this.form.personList = response.data.person.map((item) => {
            return {
              uid: Math.floor(Math.random() * 100) + 1,
              prjRole: item.prjRole,
              userIds: item.userIds.split(','),
              jobContent: item.jobContent,
              userNames: item.userNames,
            };
          });
          this.commonFun.backAreaVal(this.form, response.data);
          this.form.prjLeader1 = response.data.prjLeader
            ? response.data.prjLeader.split(',')
            : null;
          this.form.techLeader1 = response.data.techLeader
            ? response.data.techLeader.split(',')
            : null;
        });
      }
    },
