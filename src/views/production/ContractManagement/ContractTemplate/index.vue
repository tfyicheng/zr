    ...
    <div class="appi">
      <div class="appc">
        ...
        <div class="main">
          ...
          <ApplyDown ref="ApplyDown" :type="9" />
          <BeforeDown ref="BeforeDown" :type="9" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    //申请导出
    exportApply(row) {
      this.$refs.ApplyDown.downSourceName = row.templateName;
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({
        path: '/production-d/templatetInfo',
    ...
    handleEdit(row) {
      this.$router.push({
        path: '/production-d/templatetEdit',
        query: { id: row.id, data: JSON.stringify(row) },
      });
    },
    ...
    //添加
    addC() {
      this.$router.push({ path: '/production-d/templatetAdd' });
    },
  },
