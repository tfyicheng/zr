<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <!-- <div class="top">外委合同</div> -->
        <NavTop :val="['生产管理', '外委合同管理', '外委合同列表 ']" />
        <div class="main">
          <div class="p1">
            <div class="p1item">
              <el-input
          ...
          <div class="p2">
            <el-button v-hasPermi="['contract:outsourcing:add']" type="primary" @click="addC"
              >添加</el-button
            >
            <el-button
              v-hasPermi="['contract:outsourcing:exportList']"
              type="warning"
              @click="exportList"
            ...
            >
            <!-- <el-button
              v-hasPermi="['contract:outsourcing:import']"
              type="warning"
              @click="importList"
              >导入</el-button
            > -->
            ...
            <el-button
              type="danger"
              v-hasPermi="['contract:outsourcing:remove']"
              @click="deleteT"
              :disabled="chooseItems.length == 0"
              >删除</el-button
            >
            ...
            <!-- <el-button
              v-hasPermi="['contract:outsourcing:contTemplate']"
              style="width: auto"
              type="primary"
              @click="downTemplate"
              >下载导入模版</el-button
          ...
          <div class="p3">
            ...
            >
              ...
              <!-- <el-table-column prop="clientName" label="合同来源">
                <template slot-scope="scope">
                  <dict-tag :flag="false" :options="dict.type.project_src" :value="scope.row.prjSrc" />
                </template>
              </el-table-column> -->
              ...
              <el-table-column fixed="right" label="操作" width="200">
                <template slot-scope="scope">
                  ...
                  >
                  <el-button
                    v-hasPermi="['contract:plan:edit1']"
                    type="text"
                    size="small"
            ...
            <!-- <el-pagination
              ...
              :total="400"
            >
            </el-pagination>               :limit.sync="queryParams.pageSize"-->
            <pagination
              v-show="total > 0"
              :total="total"
...
import {
  ...
  outContTermination,
  outContExportList,
  ocontImportTemplateDown,
} from '@/api/data/contract';
export default {
  name: 'OutsourcingContractIndex', //外委合同列表
  ...
  methods: {
    ...
    //导出列表
    exportList() {
      outContExportList(this.queryParams);
    },
    ...
    async deleteT(row) {
      ...
      outContDelete(params.join(',')).then((res) => {
        if (res.code == 200) {
    ...
    async TerminatedT(row) {
      ...
      outContTermination(params.join(',')).then((res) => {
        if (res.code == 200) {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/production-d/outsourcingContractInfo', query: { id: row.id } });
    },

    //编辑
    handleEdit(row) {
      this.$router.push({ path: '/production-d/outsourcingContractEdit', query: { id: row.id } });
    },

    //任务单
    handleSheet(row) {
      this.$router.push({ path: '/production-d/taskSheetO', query: { id: row.id } });
    },
  },
