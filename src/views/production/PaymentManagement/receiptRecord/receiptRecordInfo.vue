<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '收票记录管理', '收票记录列表', '收票记录详情']" />
        ...
        <ApplyDown ref="ApplyDown" :type="15" />
        <BeforeDown ref="BeforeDown" :type="15" />
      </div>
    </div>
  </div>
</template>
...
<script>
import { ticketCInfo } from '@/api/data/produce';

export default {
  name: 'ReceiptRecordInfo', //收票记录信息
  dicts: ['invoice_type'],
  data() {
    return {
      maxtbheigth: '300',
  ...
  methods: {
    ...
    exportApply() {
      ...
      this.$refs.ApplyDown.downSourceName = '收票附件';
