    ...
    <div class="appi">
      <div class="appc">
        ...
        <div class="main">
          ...
          <div class="p2">
            <el-button v-hasPermi="['produce:payment0:add']" type="primary" @click="addC"
              >添加</el-button
            >
            <!-- <el-button type="warning" @click="exportT" :disabled="chooseItems.length == 0"
              >导出</el-button
            > -->
            <el-button type="warning" v-hasPermi="['produce:payment0:import']" @click="importList"
              >导入</el-button
            >

            <el-button
              v-hasPermi="['produce:payment0:importTemplate']"
              style="width: auto"
              type="primary"
              @click="downTemplate"
              >下载导入模版</el-button
            ...
            <!-- <el-button
              v-hasPermi="['produce:payment0:remove']"
              type="danger"
              @click="deleteT"
...
<script>
import { payList, payDelete, importTemplate3 } from '@/api/data/produce';
export default {
  name: 'PaymentRequestCheck', //付款申请查看
  dicts: ['payment_type'],
  data() {
    return {
      loading: true, // 遮罩层
  ...
  methods: {
    ...
    async deleteT(row) {
      ...
      payDelete(params.join(',')).then((res) => {
        if (res.code == 200) {
          this.$message.success(`删除成功`);
          this.$refs.multipleTable.clearSelection();
          this.getList();
