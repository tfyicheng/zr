<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '付款记录管理', '付款记录列表', '编辑付款申请']" />
        <div class="main">
          <div class="p0">
            <el-form :model="form" :inline="true" ref="form" :rules="ruleData">
              ...
              <el-form-item label="付 款 类 型" prop="paymentBusinessType">
                <el-select v-model="form.paymentBusinessType" placeholder="点击选择" clearable>
                  <el-option
                    v-for="item in dict.type.payment_business0"
                    :key="item.value"
                    :label="item.label"
              ...
              <el-form-item label="付 款 日 期" prop="paymentTime">
                <el-date-picker
              ...
              <el-form-item class="pline" :label="'累计付款\n金额(元)'">
                <el-input-number
              ...
              <el-form-item label="此次付款金额" prop="currentAmount">
                <el-input-number
              ...
              <el-form-item label="付 款 备 注" class="remark">
                <el-input
                  type="textarea"
                  placeholder="请输入内容"
                  v-model="form.remark"
              ...
              <el-form-item label="付 款 附 件" class="file">
                <FileChoose
                  ref="FileChoose"
        ...
        <!-- <el-dialog :visible.sync="dialogTableVisible">
          ...
          <ContList
            v-show="this.ChooseOne == 'contList'"
            :type="0"
            :Multiple="false"
            ref="contList"
          />
...
import { outContInfo, outContNodeList } from '@/api/data/contract';
import { payEdit, payInfo } from '@/api/data/produce';
import { payAmount } from '@/api/data/produce';
import { proPayMentNodeList, payNodeAmount } from '@/api/data/project';
import { custInfo } from '@/api/data/customer';
export default {
  name: 'PaymentRequestEdit', //编辑付款申请
  ...
  data() {
    return {
      ...
      ruleData: {
        ...
        nodeId: [{ required: true, message: '请选择付款节点', trigger: 'change' }],
        ...
        oppositeAccount: [{ required: true, message: '付款账户不能为空', trigger: 'blur' }],
  ...
  methods: {
      ...
      if (this.$route.query.id) {
        ...
        payInfo(this.$route.query.id).then((response) => {
          ...
          if (response.data) {
            ...
            this.getNodes(response.data.contId);
            //this.getTotalAmount(response.data.id, response.data.prjNumber);
            this.SelectNode();
          }
        });
      }
    ...
    getInfo(id) {
      if (id) {
        outContInfo(id).then((response) => {
          ...
          //结构对象排除某些字段
          let { files, ...profileData } = response.data;
          this.form = profileData;
          //区域数据回传
          this.commonFun.backAreaVal(this.form, response.data);
          payAmount(response.data.id).then((response) => {
            if (response.data) {
    ...
    getNodes(id) {
      if (id) {
        //proPayMentNodeList(1, id).then((response) => {
        outContNodeList(id).then((response) => {
          console.log(response); //添加收付款节点只能是状态1，2 参考字典可付款类型
    ...
    //   payAmount({ contId: id, prjNumber: prjNumber }).then((response) => {
