          ...
          <div class="p1">
            ...
            <div class="p1item">
              <el-select v-model="queryParams.status" placeholder="状态" clearable>
                <el-option
                  v-for="item in dict.type.out_node_status"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value + ''"
                  @input="$forceUpdate()"
...
<script>
import { payNodeList } from '@/api/data/contract';
export default {
  name: 'PaymentNodeCheck', //查看付款节点
  dicts: ['out_node_status', 'contract_status'],
  data() {
    return {
      loading: true, // 遮罩层
      chooseItems: [], // 选中项
  ...
  methods: {
    ...
    getList() {
      ...
      payNodeList(this.queryParams).then((response) => {
    ...
    handleEdit(row) {
      ...
      this.tableData.forEach((item) => {
        ...
        if (data) {
          this.$router.push({
            path: '/production-d/paymentNodeEdit',
            query: { id: data.id },
          });
        } else {
          this.$router.push({
