<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '付款节点管理', '付款节点列表', '编辑付款节点']" />
...
<script>
import { outContNodeEdit, outContNodeList } from '@/api/data/contract';
import { receiptsAmount } from '@/api/data/produce';

export default {
  name: 'PaymentNodeEdit', //编辑付款节点
  data() {
    return {
      inputTipWidth: 230,
      form: {
  ...
  methods: {
    ...
    getInfo() {
      ...
      if (this.$route.query.id) {
        ...
        outContNodeList(this.$route.query.id).then((response) => {
          //console.log(response);
          this.form = response.data;
          // resdata = JSON.parse(JSON.stringify(response.data));
          // receiptsAmount({ contId: resdata.id, prjNumber: resdata.prjNumber }).then((response) => {
