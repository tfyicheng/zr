<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['生产管理', '客户管理', '客户列表', '编辑客户信息']" />
        <div class="main">
          <div class="p0">
            <el-form :model="form" :inline="true" ref="form" :rules="ruleData">
              ...
              <el-form-item label="所 在 区 域" prop="prjArea">
        ...
        <div class="main2">
          ...
          <div class="m2main" v-show="nodeShow">
            <div class="m2mt" v-show="form.clientContacts && form.clientContacts.length > 0">
...
<script>
import { custUpdate, custInfo, custClientList } from '@/api/data/customer';

export default {
  name: 'CustomerManagementEdit', //客户管理-编辑
    ...
    getInfo() {
      if (this.$route.query.id) {
        ...
        custInfo(this.$route.query.id).then((response) => {
          ...
          if (response.data) {
            if (response.data.clientContacts) {
              this.form = response.data;
            } else {
              let clientContacts = [];
              this.form = { clientContacts, ...response.data };
            }

            custClientList(this.$route.query.id).then((response) => {
              if (response.data) {
                this.form.clientContacts = response.data.map((c) => {
                  return {
                    name: c.name,
                    dept: c.dept,
                    phone: c.phone,
            ...
            // this.form.prjArea = [
            //   response.data.areaProvince,
            //   response.data.areaCity,
            //   response.data.areaRegion,
            // ];
            // this.form.addrArea = [
