...
import { carApplyListInfoo, carApplyCancel } from '@/api/data/car';
export default {
  name: 'CarReceiveCheck', //查看领用审批
  ...
  methods: {
    ...
    getList(queryParam) {
      ...
      carApplyListInfoo(this.queryParams).then((response) => {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/car-d/carReceiveInfo', query: { id: row.id } });
    },

    //处理
    handleEdit(row) {
      this.$router.push({
        path: '/car-d/carReceiveHandle',
