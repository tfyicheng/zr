    ...
    <div class="appi">
      <div class="appc">
        ...
        <div class="main">
          <div class="p1">
            ...
            <div class="p1item">
              <el-select v-model="queryParams.businessType" placeholder="业务类型" clearable>
                <el-option
                  v-for="item in dict.type.leave_type"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
          ...
          <div class="p2">
            <el-button v-hasPermi="['person:apply:add1']" type="primary" @click="addC"
              >添加</el-button
            >
            <el-button
              v-hasPermi="['person:apply:export1']"
              type="warning"
              @click="exportList"
          ...
          <div class="p3">
            ...
            >
              ...
              <el-table-column fixed="right" label="操作" :show-overflow-tooltip="false">
                <template slot-scope="scope">
                  <el-button
                    v-hasPermi="['person:apply:query1']"
...
import { listAskForLeave, personOutRevokeo, userLeaveExportList } from '@/api/data/person';
export default {
  name: 'PersonLeaveCheck', //查看人员请假
  dicts: ['approval_status', 'leave_type'],
  data() {
    return {
      depts: [], //部门项
      loading: true, // 遮罩层
  ...
  methods: {
    ...
    //导出列表
    exportList() {
      userLeaveExportList(this.queryParams);
    },
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/person-d/personLeaveInfo', query: { id: row.id } });
    },
