<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['人员管理', '人员合同管理', '人员合同列表']" />
        <div class="main">
          <div class="p1">
            <div class="p1item">
              <el-input v-model="queryParams.contId" placeholder="合同编号" clearable></el-input>
            </div>
            <div class="p1item">
            ...
            <div class="p1item">
              <el-input
                v-model="queryParams.secondParty"
                placeholder="用户名称"
                clearable
              ></el-input>
            </div>
            ...
            <div class="p1item">
              <el-select v-model="queryParams.dueStatus" placeholder="到期状态" clearable>
                <el-option
                  v-for="item in commonFun.dueStatus"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value + ''"
                  @input="$forceUpdate()"
          ...
          <div class="p2">
            <el-button v-hasPermi="['contract:labor:add']" type="primary" @click="addC"
              >添加</el-button
            >
            <el-button
              v-hasPermi="['contract:labor:export']"
              type="warning"
              @click="exportList"
            ...
            >
            <el-button
              v-hasPermi="['contract:labor:remove']"
              type="danger"
              @click="deleteT"
          ...
          <div class="p3">
            ...
            >
              ...
              <el-table-column label="到期状态">
                <template slot-scope="scope">
                  <span v-if="scope.row.dueStatus === 0" style="color: #07c15f">{{
                    commonFun.dueStatus[scope.row.dueStatus].label
                  }}</span>
                  <span v-else-if="scope.row.dueStatus === 1" style="color: red">{{
                    commonFun.dueStatus[scope.row.dueStatus].label
                  }}</span>
                  <span v-else-if="scope.row.dueStatus === 2" style="color: #fac06a">{{
                    commonFun.dueStatus[scope.row.dueStatus].label
                  }}</span>
                </template>
              </el-table-column>
...
<script>
import { laborContList, laborContDelete, labContExportList } from '@/api/data/contract';
export default {
  name: 'LaborContractIndex', //劳动合同
  ...
  data() {
    return {
      ...
      tableData: [
        // {
        //   id: 13, //证书ID
        //   certNumber: '202312201245218', //证书编号
        //   certName: '证书名称', //证书名称
        //   certParentType: 0, //证书大类
        //   certChildType: 0, //证书中类
        //   certLevel: null, //证书等级
        //   certType: null, //证书种类
        //   authority: '颁发机构xxx', //颁发机构
        //   issueDate: '2023-12-15', //颁发时间
        //   userName: null, //用户
        //   deptName: '珠荣工程设计有限公司', //组织
        //  "files": [
        // {
        // "primaryId": null, //附件列表
        ...
        // {
        // "primaryId": null, //附件列表
        // "annexId": 1735214359929446400, //文件ID
        // "originalName": "all.log.2023-02-28.0.gz" //文件名称
        // }
        //]
        // },
      ], //证书信息表格数据
  ...
  methods: {
    //查询列表
    getList() {
      if (this.$route.query.dueStatus != null) {
        this.queryParams.dueStatus = this.$route.query.dueStatus;
        this.$router.push({ path: this.$router.currentRoute.path, query: null });
      }

      console.log(this.queryParams);
      laborContList(this.queryParams).then((response) => {
    ...
    //导出列表
    exportList() {
      labContExportList(this.queryParams);
    },
    ...
    async deleteT(row) {
      ...
      laborContDelete(params.join(',')).then((res) => {
        if (res.code == 200) {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/person-d/laborContractInfo', query: { id: row.id } });
    },

    //编辑
    handleEdit(row) {
      this.$router.push({ path: '/person-d/laborContractEdit', query: { id: row.id } });
    },

    //添加
    addC() {
      this.$router.push({ path: '/person-d/laborContractAdd' });
    },
  },
