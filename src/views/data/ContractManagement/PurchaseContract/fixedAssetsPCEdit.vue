          ...
          <div class="m2main" v-show="paymentShow">
            ...
            >
              <div class="m2mt">
                ...
                <div class="m2mb">
                  <el-form-item label="" prop="tradePrice">
                    <el-input-number
...
import { purchaseContUpdate, purchaseContInfo } from '@/api/data/contract';
export default {
  name: 'FixedAssetsPCEdit', //编辑固定资产采购合同
  components: { PersonList, DeptList, SupList, MatList },
  data() {
    return {
      form: {
        annexFile: [],
    ...
    getInfo() {
      if (this.$route.query.id) {
        ...
        purchaseContInfo(this.$route.query.id).then((response) => {
          ...
          //处理展示
          this.form.ChooseUser = response.data.applyUser; //选择的用户名称
          this.form.ChooseDept = response.data.applyDept; //选择的部门名称
          this.form.ChooseSup = response.data.supplierName; //选择的供应商名称
          this.form.trades = response.data.trades.map((item) => {
            return {
              materialId: item.materialId,
              materialName: item.materialName,
              tradeNum: item.contQuantity,
