  ...
  <div class="appo">
    <div class="appi">
      <div class="appc">
        ...
        <NavTop :val="['资料管理', '合同管理', '项目合同列表', '变更项目合同']" />
          ...
          <div class="p0">
            <el-form :model="form" :inline="true" ref="form" :rules="ruleData">
              ...
              <el-form-item label="项 目 来 源" prop="prjSrc">
                <el-select v-model="form.prjSrc" placeholder="点击选择" clearable>
                  <el-option
                    v-for="item in dict.type.project_src"
                    :key="item.value"
                    :label="item.label"
              ...
              <el-form-item label="参 与 部 门">
                <!-- <el-input
                  ...
                  placeholder="点击选择"
                  @click.native="chooseDept('form',0)"
                  readonly
                >
              ...
              <el-form-item label="合同负责人" prop="userId">
                <!-- <el-input
                  ...
                  placeholder="点击选择"
                  @click.native="chooseUser('form', 0)"
                  readonly
                >
                  <svg-icon
                    slot="suffix"
        ...
        <!-- @getClie="getClie($event, 33)" event获取传递值 -->
        <div class="main2">
          <div class="m2top">
            客户信息
            <div class="m2topm">
          ...
          <div class="m2main" v-show="c.flag">
            <div class="p0">
              <el-form :model="c" :inline="true" :ref="`refCont${index}`" :rules="ruleData">
                ...
                <el-form-item label="参 与 部 门">
                  <DeptChoose :data.sync="c.deptId" :backVal="form.deptName" />
                </el-form-item>
        ...
        <el-dialog :visible.sync="dialogTableVisible">
          ...
          <DeptList v-show="this.ChooseOne == 'dList'" :Multiple="true" ref="dList" />
        ...
        <!-- <PersonDialog :Multiple="false" @getChoose="getUser" ref="PersonDialog" /> -->
      </div>
    </div>
  </div>
</template>
...
import { projectContUpdate, projectContInfo } from '@/api/data/contract';
import { custInfo } from '@/api/data/customer';
export default {
  name: 'ProjectContractEdit', //变更项目合同信息
  dicts: [
    'cont_type',
    'project_scale',
    'project_type',
  ...
  components: { PersonList, DeptList, ProjList, ClieList, PersonDialog },
  data() {
    return {
      form: {
        telephone: null,
        ...
        //   {
        //     primaryId: null,
        //     annexId: '1775334518490259456',
        //     originalName: '屏幕截图62648.png',
        //   },
        //   {
        //     primaryId: null,
        //     annexId: '1775335794816966656',
        //     originalName: '屏幕截图62649.png',
        //   },
        // ],
        paymentNodes: [],
        //  ChoosePri: '', //选择的项目名称
      ...
      ruleData: {
        ...
        mailArea: [{ required: true, message: '区域不能为空', trigger: ['blur', 'change'] }],
  ...
  methods: {
    ...
    getPri(data) {
      ...
      if (data && data.length > 0) {
        this.form.ChoosePri = data[0].prjName;
        this.form.prjNumber = data[0].prjNumber;
      }
    },

    //选择部门id
    chooseDept(form, flag) {
      this.flagOne.form = form;
      this.flagOne.flag = flag;
      this.dialogTableVisible = true;
        ...
        if (data && data.length > 0) {
          if (Array.isArray(data)) {
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              this.form.deptIds.push(data[i].deptId);
            }
            this.form.ChooseDept = str;
          }
        }
      }
      if (this.flagOne.form == 'cont') {
        this.form.deptIds = [];
        if (data && data.length > 0) {
          if (Array.isArray(data)) {
            let str = '';
    ...
    chooseClient() {
      ...
      this.$nextTick(() => {
        this.$refs.cList.$off('getChoose', this.getClient);
        this.$refs.cList.$on('getChoose', this.getClient);
    ...
    getClient(data) {
      this.dialogTableVisible = false;
      if (data && data.length > 0) {
        this.form.ChooseClient = data[0].name;
        this.form.clientId = data[0].id;
        this.form.contacts = data[0].userName;
        ...
        if (res) {
          ...
          Promise.all([
            ...
            if (res) {
              ...
              if (this.form.settlementAmount <= 0) {
                this.form.settlementAmount = null;
              }
              if (this.form.totalInvestment <= 0) {
                this.form.totalInvestment = null;
              }
              //addrArea: [],//注册
              // mailArea: [],//邮寄
              // prjArea: [],//项目
              this.form.prjArea2 = this.form.prjArea;
              this.commonFun.ConverAreaVal(this.form);
              projectContUpdate(this.form)
                .then((res) => {
                  console.log(res);
  ...
  computed: {
    ...
    calVal() {
      return function (val) {
        if (this.form.settlementAmount && val) {
          return this.form.settlementAmount * (val / 100);
        } else {
          if (this.form.totalAmount && val) {
            return this.form.totalAmount * (val / 100);
          } else {
            return '请先输入结算金额或合同金额';
          }
        }
      };
    },
