<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['项目管理', '项目资料归档管理', '项目资料归档列表', '编辑资料归档']" />
        <div class="main">
          <div class="p0">
        ...
        <div class="p3">
          ...
          <el-button type="primary" v-show="!isComplete" style="width: auto" @click="save(true)"
            >确认并完成归档</el-button
          >
        </div>
...
<script>
import { projDataInfo, prjDataEditInfo, prjDataUpdate, prjDataComplete } from '@/api/data/project';
  ...
  methods: {
    getInfoFrist() {
      if (this.$route.query.complete) {
        this.isComplete = this.$route.query.complete;
      }
      if (this.$route.query.id) {
        this.flagId = this.$route.query.id;
        prjDataEditInfo(this.$route.query.id).then((response) => {
          console.log(response);
          this.form = response.data;
          if (response.data.prjPhaseData != null) {
            //this.form.prjPhaseData = {...response.data.prjPhaseData,...this.form.prjPhaseData}
            this.form.prjPhaseData = response.data.prjPhaseData.map((c) => {
              return { ...c, flag: true, uid: Math.floor(Math.random() * 100) + 1 };
            });

            this.form.prjPhaseData.forEach((c) => {
              (c.prjYear = c.prjYear + ''),
                c.prjData.forEach((cc) => {
                  let upf = this.commonFun.splitStr(
                    cc.files.map((ccc) => {
                      return ccc.originalName;
                    }),
                  );

                  cc.upfiles = upf ? upf : null;
                  cc.fileIds = cc.fileId.split(',');
                  cc.uuid = Math.floor(Math.random() * 1000) + 1;
                });
            });
          } else {
            this.form.prjPhaseData = [];
          }
    ...
    async upFileForm() {
      ...
      for await (let item of vf) {
        ...
        console.log('over:', promise, this.$refs[item.uuid][0].upStatus);
        //  await new Promise((resolve) => setTimeout(resolve, 2000));
        // while (this.$refs[item.uuid][0].upStatus != 0) {
        while (true) {
          console.log('等待组件上传完毕', item.uuid);
          await new Promise((resolve) => setTimeout(resolve, 2000));
          if (this.$refs[item.uuid][0].number == 0) {
            console.log('组件上传完毕', item.uuid);
            break;
          }
        }
      }
    ...
    getUlist(data) {
      ...
      // if (this.$refs.uList.number == 0 && data.type == 0) {
      //   this.save2();
      // }
    },
    ...
    save(flag = false) {
      ...
      for (const i in vf) {
        if (!vf[i].upfiles) {
          this.$message.error(`资料 [${vf[i].dataName ? vf[i].dataName : '未命名'}] 附件不能为空`);
          return;
        }
      }
    ...
    save3() {
      prjDataComplete(this.form.id).then((res) => {
        if (res.code == 200) {
