<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop :val="['项目管理', '项目资料归档管理', '项目资料归档列表']" />
        <div class="main">
          <div class="p1">
            <div class="p1item">
            ...
            <div class="p1item">
              <AddrCascader ref="AddrCascader" :data.sync="queryParams.area" />
            </div>
          ...
          <div class="p2">
            <el-button v-hasPermi="['project:data:place:add']" type="primary" @click="addC"
              >添加</el-button
            >
          ...
          <div class="p3">
            ...
            >
              ...
              <el-table-column prop="contNumber" label="关联合同编号"> </el-table-column>
              ...
              <el-table-column prop="prjArea" label="区域"> </el-table-column>
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    ...
                    v-hasPermi="['project:data:place:query']"
                  ...
                  >
                  <el-button
                    v-show="scope.row.canEdit && scope.row.children != null"
                    type="text"
                    size="small"
...
<script>
import { prjDataList, prjDataDelete } from '@/api/data/project';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      prjDataList(this.queryParams).then((response) => {
        this.tableData = response.rows.map((c) => {
          if (c.children && c.children.length > 0) {
            c.contNumber = c.children[0].contNumber;
            c.prjPhase = c.children[0].prjPhase;
            c.prjYear = c.children[0].prjYear;
          }
          return c;
        });
    ...
    async deleteT(row) {
      ...
      if (row.children == null) {
        prjDataDelete(1, row.id).then((res) => {
          if (res.code == 200) {
        ...
        prjDataDelete(0, row.id).then((res) => {
          if (res.code == 200) {
            this.$message.success(`删除成功`);
            this.$refs.multipleTable.clearSelection();
            this.getList();
    ...
    handleCheck(row) {
      ...
      //         this.$router.push({
      //           path: '/project-d/projectDataInfo',
      //           query: { id: item.id, flag: row.canEditOrDel ? 1 : 0 },
      //         });
      //       }
      //     });
      //   }
      ...
      this.$router.push({
        path: '/project-d/projectDataInfo',
        query: { id: row.id, flag: row.canEditOrDel ? 1 : 0 },
      });
    },
    ...
    handleEdit(row) {
      ...
      //       if (item2 == row) {
      //         //console.log('父节点', item); //输出父节点
      //         this.$router.push({ path: '/project-d/projectDataEdit', query: { id: item.id } });
      //       }
      //     });
      //   }
      // });
      ...
      this.$router.push({
        path: '/project-d/projectDataEdit',
        query: { id: row.id, complete: row.complete },
      });
    },
