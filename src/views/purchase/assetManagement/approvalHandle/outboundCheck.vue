    ...
    <div class="p3">
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" :show-overflow-tooltip="false">
          <template slot-scope="scope">
            <el-button
              v-hasPermi="['property:stock3:info']"
            ...
            <el-button
              v-show="scope.row.approves && scope.row.approves.length >= 2"
              @click="handleCheck(scope.row)"
              type="text"
              size="small"
              style="font-size: 14px"
...
import { stockOutAList, storeList, stockApproveBack } from '@/api/data/asset';
export default {
  name: 'outboundCheck', //查看出库审批
  dicts: ['approval_status', 'out_type', 'store_out_type'],
  data() {
    return {
      depts: [], //部门项
      stores: [], //仓库项
  ...
  methods: {
    ...
    getList(queryParam) {
      ...
      stockOutAList(this.queryParams).then((response) => {
    ...
    /** 搜索按钮操作 */
    handleQuery() {
      this.$router.push({ path: '/purchase-d/approvalHandleCheck', query: null });
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({
        path: '/purchase-d/outboundHandle',
