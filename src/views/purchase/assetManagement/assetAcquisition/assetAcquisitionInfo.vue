<template class="app-container">
  <div class="appo">
    <div class="appi">
      <div class="appc">
        <NavTop
          :val="['资产采购管理', '固定资产领用管理', '固定资产领用列表', '固定资产领用详情']"
        />
        <div class="main">
          <div class="mainl">
            <div class="p1">
              <!-- <div class="p1item">
                <span>领 用 编 号</span>
                <span><InputTip :width="inputTipWidth" :text="form.id"></InputTip></span>
              </div> -->
              ...
              <!-- <div class="p1item">
                <span>领 用 地 点</span>
                <span> <InputTip :width="inputTipWidth" :text="form.useAddr"></InputTip></span>
              </div> -->

              <!-- <div class="p1item">
                <span>出库类型</span>
                <span>{{ form.outType ? dict.type.store_out_type[form.outType].label : null }}</span>
              </div> -->
        ...
        <div class="main2">
          ...
          <div class="m2main" v-show="paymentShow">
            ...
            >
              ...
              <!-- <el-table-column prop="" label="购入时间"> </el-table-column>
        ...
        <div class="p3">
          ...
          <el-button
            v-show="approves.find((c) => c.status == 1) && approves.find((c) => c.status == 1).show"
            @click="handleWork(1)"
            >通过</el-button
          >
          <el-button
            v-show="approves.find((c) => c.status == 2) && approves.find((c) => c.status == 2).show"
            @click="handleWork(2)"
            >驳回</el-button
          >
...
<script>
import { stockInfo, stockApproveBack, stockUseC0Info } from '@/api/data/asset';

export default {
  name: 'AssetAcquisitionInfo', //资产领用信息
  dicts: ['project_scale', 'store_out_type', 'unit_of_measurement', 'property_type2'],
  data() {
    return {
      maxtbheigth: '300',
      paymentShow: true,
  ...
  methods: {
    ...
    getInfo() {
      if (this.$route.query.id) {
        stockUseC0Info(this.$route.query.id).then((response) => {
