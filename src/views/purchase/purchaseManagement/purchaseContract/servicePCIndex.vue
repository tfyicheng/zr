...
export default {
  ...
  methods: {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/purchase-d/servicePCInfo', query: { id: row.id } });
    },

    //编辑
    handleEdit(row) {
      this.$router.push({ path: '/purchase-d/servicePCEdit', query: { id: row.id } });
    },

    //添加
    addC() {
      this.$router.push({ path: '/purchase-d/servicePCAdd' });
    },
  },
