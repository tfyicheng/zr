<template class="app-container">
  <div class="main">
    ...
    <div class="p2">
      <el-button v-hasPermi="['contract:propertyBuy:add']" type="primary" @click="addC"
        >添加</el-button
      >
      <el-button
        v-hasPermi="['contract:propertyBuy:exportList']"
        type="warning"
        @click="exportList"
      ...
      >
      <el-button
        v-hasPermi="['contract:propertyBuy:termination']"
        type="danger"
        @click="TerminatedT"
      ...
      >
      <el-button
        v-hasPermi="['contract:propertyBuy:remove']"
        type="danger"
        @click="deleteT"
        :disabled="chooseItems.length == 0"
        >删除</el-button
    ...
    <div class="p3">
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" width="150">
          <template slot-scope="scope">
            <el-button
              v-hasPermi="['contract:propertyBuy:query']"
            ...
            >
            <el-button
              v-hasPermi="['contract:propertyBuy:edit']"
...
import {
  purchaseContList,
  purchaseContDelete,
  purchaseContTermination,
  buyContExportList,
} from '@/api/data/contract';
export default {
  name: 'FixedAssetsPCIndex', //固定资产采购合同
  ...
  methods: {
    ...
    /** 搜索按钮操作 */
    handleQuery() {
      this.$router.push({ path: '/purchase-d/purchaseContractIndex', query: null });
    ...
    //导出列表
    exportList() {
      buyContExportList(this.queryParams);
    },
    ...
    async deleteT(row) {
      ...
      purchaseContDelete(params.join(',')).then((res) => {
        if (res.code == 200) {
    ...
    async TerminatedT() {
      ...
      purchaseContTermination(params.join(',')).then((res) => {
        if (res.code == 200) {
    ...
    handleCheck(row) {
      console.log(row);
      this.$router.push({ path: '/purchase-d/fixedAssetsPCInfo', query: { id: row.id } });
    },

    //编辑
    handleEdit(row) {
      this.$router.push({ path: '/purchase-d/fixedAssetsPCEdit', query: { id: row.id } });
    },

    //添加
    addC() {
      this.$router.push({ path: '/purchase-d/fixedAssetsPCAdd' });
    },
  },
